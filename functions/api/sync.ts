...
const contentFlagsFrom = (model: HubModel) => [...new Set((`${model.id || ""} ${(model.tags || []).join(" ")}`.match(/nsfw|porn|sex|uncensored|explicit|nudity|18\+|adult/gi) || []).map((flag) => flag.toLowerCase()))];
...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  ...
  const statements = candidates.map((model) => {
    ...
    const contentFlags = contentFlagsFrom(model);
    return env.DB.prepare(`INSERT INTO models (id, slug, name, author, hf_url, type, base_family, base_model, purpose, description, best_for, license, commercial_use, lora_rank, file_size_mb, downloads, likes, rating, review_count, docs_quality, safetensors, content_warning, content_flags, compatibility_summary, updated_at, quality_score) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?, NULL, 0, 1, ?, ?, ?, '[]', ?, ?) ON CONFLICT(id) DO UPDATE SET downloads=excluded.downloads, likes=excluded.likes, updated_at=excluded.updated_at, safetensors=excluded.safetensors, license=excluded.license, base_model=excluded.base_model, type=excluded.type, base_family=excluded.base_family, content_warning=excluded.content_warning, content_flags=excluded.content_flags`).bind(id, id.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""), id.split("/").pop() || id, id.split("/")[0] || "unknown", `https://huggingface.co/${id}`, type, familyFrom(base, id), base, purpose, "Hubから同期されたLoRA。READMEと実使用レビューで内容を補完できます。", `${type} の試作と比較`, model.cardData?.license || "Unknown", Number(model.downloads || 0), Number(model.likes || 0), file ? 1 : 0, contentFlags.length ? 1 : 0, JSON.stringify(contentFlags), model.lastModified || new Date().toISOString(), Number(model.downloads || 0) / 1000);
  });
